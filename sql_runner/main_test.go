...
func TestResolveSqlRoot(t *testing.T) {
	...
	str, err = resolveSqlRoot(SQLROOT_PLAYBOOK, "../integration/resources/good-postgres.yml", "", false)
	assert.NotNil(str)
	assert.Nil(err)
	...
	str, err = resolveSqlRoot(SQLROOT_PLAYBOOK_CHILD, "../integration/resources/good-postgres.yml", "", false)
	assert.NotNil(str)
	assert.NotNil(err)
	assert.Equal("", str)
